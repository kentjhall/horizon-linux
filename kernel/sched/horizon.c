...
__pick_next_task_horizon(struct rq *rq, struct task_struct *prev,
...
static struct task_struct *
pick_next_task_horizon(struct rq *rq, struct task_struct *prev,
		struct pin_cookie cookie)
{
	struct task_struct *picked = __pick_next_task_horizon(rq, prev, cookie);
	if (picked)
...
select_task_rq_horizon(struct task_struct *p, int cpu, int sd_flag, int flags)
{
	...
	for_each_cpu(c, tsk_cpus_allowed(p)) {
		rq = cpu_rq(c);
		hzn_rq = &rq->hzn;
...
static void
set_curr_task_horizon(struct rq *rq)
{
...
#endif
	.set_curr_task          = set_curr_task_horizon,
