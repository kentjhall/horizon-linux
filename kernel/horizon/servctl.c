	        ...
	        unsigned long, arg5)
{
	...
	case HZN_SCTL_WRITE_BUFFER_TO:
	{
		...
		down_read(&requester_mm->mmap_sem);
		ret = get_user_pages_remote(tsk, requester_mm, arg1, nr_pages,
				            cmd == HZN_SCTL_WRITE_BUFFER ? FOLL_WRITE : 0,
					    pages, NULL);
		up_read(&requester_mm->mmap_sem);
		mmput(requester_mm);
...
out:
	...
	case HZN_SCTL_MAP_MEMORY:
	{
		...
		ret = get_user_pages_remote(
				current->hzn_session_request->requester,
				requester_mm, arg1, nr_pages, 0, pages, NULL);
		up_read(&requester_mm->mmap_sem);
